...
[![Security](https://github.com/E339-art/stripe/actions/workflows/security.yml/badge.svg)](https://github.com/E339-art/stripe/actions/workflows/security.yml)
...
The official `stripe-python` library is battle-tested, but it was designed before modern Python patterns like native async/await, Pydantic data validation, and comprehensive type annotations became standard. **stripewrap** fills that gap.

| Feature | `stripe-python` | `stripewrap` |
|---|:---:|:---:|
| Native async/await | Partial | **Full** |
| Pydantic v2 type-safe models | No | **Yes** |
| Typed exception hierarchy | No | **Yes** |
| Auto-pagination | Manual | **Automatic** |
| Retry with exponential backoff + jitter | No | **Yes** |
| Easy testing (no monkey-patching) | No | **Yes** |
| IDE autocompletion on responses | Partial | **Full** |
| Python 3.10 — 3.13 | Yes | **Yes** |
...
Under the hood, the cursor (`starting_after`) advances automatically on every page. No `while has_more` loops. No manual `starting_after` bookkeeping.
...
`construct_event` verifies the HMAC-SHA256 signature **and** rejects replayed events older than 5 minutes — protecting against replay attacks out of the box.

---

## Testing Your Integration

Because `StripeClient` uses `httpx` under the hood, testing is trivial — no monkey-patching required:
...
stripewrap uses Python's standard `logging` module. Enable debug logging to see retry attempts and webhook verification details:

```python
import logging
logging.getLogger("stripewrap").setLevel(logging.DEBUG)
```
...
| PaymentIntents | Yes | Yes | Yes |
| Charges | Yes | Yes | Yes |
| Refunds | Yes | Yes | Yes |
| PaymentMethods | Yes | Yes | — |
...
├── client.py            # Synchronous StripeClient
├── async_client.py      # Asynchronous AsyncStripeClient
├── models.py            # Pydantic v2 models for all resources
├── exceptions.py        # Typed exception hierarchy
├── pagination.py        # Auto-pagination iterators
...
| Example | Description |
|---|---|
| [`create_payment_intent.py`](examples/create_payment_intent.py) | Sync payment flow with error handling |
| [`async_payment_flow.py`](examples/async_payment_flow.py) | Async payment flow with `asyncio.gather` |
| [`webhook_handler.py`](examples/webhook_handler.py) | Flask webhook receiver with signature verification |
